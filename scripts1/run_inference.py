...
# run_inference.py
# 适配 infer.py v1.1 更新2, 回退到不需要强行提取json的版本
from __future__ import annotations
...
def build_model_input(row: Dict[str, Any], text_field: str) -> Dict[str, str]:
    text = str(row.get(text_field, "")).strip()
    book_title = row.get("book_titles") or row.get("book_title") or row.get("title") or ""
    review = row.get("review") or row.get("full_review") or row.get("review_text") or ""
    if not review:
        review = text
    return {
        "text": text,
...
def batch_inference(input_path: str, output_path: str, args: argparse.Namespace):
    ...
    if args.compress:
        print("Compression: ON (Rate: {:.2f})".format(args.rate))
    else:
    ...
    with open(input_path, "r", encoding="utf-8") as f_in, \
        ...
        for line in tqdm(f_in, desc="Inference"):
            ...
            try:
                ...
                if len(buffer_inputs) >= batch_size:
                    # 1. 执行批量推理
                    # === 将 args.adapter 传入 classify_text ===
                    predictions = classify_text(
                        buffer_inputs, 
                        content_type=args.content_type, 
                        adapter_path=args.adapter,
...
def main() -> None:
    ...
    # === 接收 adapter 路径 ===
    parser.add_argument("--adapter", default=None, help="Path to LoRA adapter folder (optional)")
    parser.add_argument("--compress", action="store_true", help="是否开启 LLMLingua 提示词压缩")
    parser.add_argument("--rate", type=float, default=0.5, help="压缩保留比例 (例如 0.5 代表只保留 50% 的 Token)")
